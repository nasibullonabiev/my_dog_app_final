...
import 'package:my_dog_app_final/services/interceptor_service.dart';
import 'package:my_dog_app_final/services/log_service.dart';
...
class NetworkService {
  ...
  static Future<void> GETDOWNLOADWEB(String image, String name) async {
    try {
      ...
      AnchorElement a =
      AnchorElement(href: 'data:image/jpeg;base64,$base64data');
      a.download = '$name.jpg';
      a.click();
      a.remove();
    } catch (e) {
  ...
  static Map<String, String> paramsOneImage(String imageId) {
    Map<String, String> map = {'image_id': imageId};
    return map;
  }
  ...
  static Map<String, String> paramsMyImage(
      {List<String>? breedsId,
        String order = "RANDOM",
        int limit = 10,
        int page = 0,
    ...
    if (originalFileName != null)
      map.addAll({"original_filename": originalFileName});
    if (includeVote != null)
      map.addAll({"include_vote": includeVote.toString()});
    if (includeFavorite != null)
      map.addAll({"include_favorite": includeFavorite.toString()});
  ...
  static Map<String, dynamic> bodyVotes(
    ...
    Map<String, dynamic> map = {"image_id": imageId, "value": value};
